...
# =========================================================================
# 2. XỬ LÝ TOÀN BỘ ẢNH & THUẬT TOÁN ĐO TẦN SỐ DII KÉO DÀI
# =========================================================================
def process_ecg_dataset(pil_img: Image.Image):
    ...
    if dii_long_roi.shape[0] > 15:
        dii_sig = extract_signal_from_roi(dii_long_roi)
        cut_st = int(len(dii_sig) * 0.04)
        sig_srch = dii_sig[cut_st:]
        amp_s = np.max(sig_srch) - np.min(sig_srch) if len(sig_srch) > 0 else 0
        if amp_s > 4.0:
            pks, _ = find_peaks(dii_sig, distance=int(px_per_sec * 0.22), prominence=amp_s * 0.25)
            pks = [p for p in pks if p > cut_st]
            if len(pks) >= 2:
    ...
    if len(dii_rrs) >= 2:
        mean_rr = float(np.median(dii_rrs))
        rr_cv = float(np.std(dii_rrs)) / mean_rr if mean_rr > 0 else 0.0
    ...
    return {
        ...
        "mean_rr": mean_rr,
        "rr_cv": rr_cv,
        "rr_list": dii_rrs if len(dii_rrs) >= 2 else valid_rrs,
        "qrs": qrs_final,
        "pr": pr_final,
...
class ECGClinicalAnalyzer:
    ...
    def evaluate_artifacts(self):
        ...
        if d1_all_neg and avr_pos:
            self.findings.append(("Sai lệch điện cực", "Nghi ngờ Đảo ngược điện cực tay phải - tay trái (LA/RA): DI đảo ngược hoàn toàn (P, QRS, T âm), aVR dương tính"))
        ...
        if v2.get("p_amp", 0.0) < 0 or v2.get("t_morph") in ["biphasic_pos_neg", "biphasic_neg_pos"] and (v2.get("r_amp", 0.0) < 3.0):
            self.findings.append(("Sai lệch điện cực", "Gợi ý Đặt điện cực V1-V2 quá cao (KLS 2 hoặc 3): Sóng P ở V2 âm hoặc hai pha (Dễ gây giả sóng Q, giả RBBB hoặc giả Brugada)"))
    ...
    def evaluate_conduction(self):
        ...
        if m_av == "Block AV độ I":
            ...
            av_diag = "Block nhĩ thất độ II Mobitz type 1 (Chu kỳ Wenckebach): Khoảng PR tăng dần cho đến khi có 1 sóng P không dẫn"
        elif m_av == "Block AV độ II Mobitz 2":
            av_diag = "Block nhĩ thất độ II Mobitz type 2: Khoảng PR cố định, có sóng P không dẫn đột ngột (RR không dẫn = 2 x RR bình thường)"
        ...
        if m_sa == "Block xoang nhĩ độ II Type 1":
            ...
            self.findings.append(("Block Xoang Nhĩ", "Block xoang nhĩ độ II type 2: Khoảng PP cố định, có khoảng nghỉ đột ngột bằng bội số nguyên của PP"))
        ...
        if m_bbb == "Block nhánh phải hoàn toàn (Complete RBBB)":
            ...
            lbbb_type = "Block nhánh trái hoàn toàn (Complete LBBB): QRS ≥ 0.12s, V5-V6/DI/aVL sóng R đơn pha có móc, V1 dạng QS/rS"
    ...
    def evaluate_hypertrophy(self):
        ...
        if m_atrial == "Lớn nhĩ phải (P phế)":
        ...
        elif m_atrial == "Lớn nhĩ trái (P nhĩ)":
            self.findings.append(("Lớn buồng tim", "Lớn nhĩ trái (P nhĩ): Sóng P ở DII dạng 2 đỉnh rộng > 0.11s, pha âm ở V1 rộng > 0.04s và sâu > 1 mm"))
        elif m_atrial == "Lớn hai nhĩ":
            self.findings.append(("Lớn buồng tim", "Lớn hai nhĩ: Sóng P vừa cao ≥ 2.5 mm vừa rộng ≥ 0.12s ở DII"))
        elif m_atrial == "Tự động":
            ...
            is_rah = (max_p_limb >= 2.5) or (v1_pos_p > 1.5)
            is_lah = (p_dur_d2 > 0.11 and p_notched_d2) or (v1_neg_p > 1.0 and v1_neg_dur > 0.04)
            ...
            if is_rah and is_lah:
                ...
                self.findings.append(("Lớn buồng tim", f"Lớn nhĩ phải (P phế): Sóng P cao {max_p_limb:.1f} mm (≥ 2.5 mm ở DII/DIII/aVF)"))
            elif is_lah:
        ...
        if has_lvh_strain:
            self.re_score += 3
        if m_atrial == "Lớn nhĩ trái (P nhĩ)":
            self.re_score += 3
        ...
        if r_v1 >= 7.0:
            rvh_signs.append(f"Sóng R ưu thế ở V1 (R={r_v1:.1f} mm)")
        ...
        if katz_wachtel_val > 50.0:
            self.findings.append(("Phì đại buồng tim", f"Dày hai thất theo tiêu chuẩn Katz-Wachtel: Tổng R + S ở V2-V5 = {katz_wachtel_val:.1f} mm (> 50 mm)"))
    ...
    def analyze_all(self):
        ...
        if not any(k in f[0] for f in self.findings for k in ["Rối loạn nhịp", "Nhịp nhanh", "Block nhĩ thất", "Hội chứng kích thích sớm"]):
            if self.hr > 100:
                self.findings.append(("Nhịp học", f"Nhịp nhanh xoang: Tần số {self.hr} lần/phút (chuẩn DII)"))
            elif self.hr < 60:
                self.findings.append(("Nhịp học", f"Nhịp chậm xoang: Tần số {self.hr} lần/phút (chuẩn DII)"))
            else:
...
with col1:
    ...
    if use_manual_override:
        ...
        st.markdown("##### 🫀 Phì đại buồng tim (Lớn nhĩ & Dày thất)")
        manual_data["atrial_override"] = st.selectbox(
            "Đánh giá Lớn nhĩ",
            ["Tự động", "Bình thường", "Lớn nhĩ phải (P phế)", "Lớn nhĩ trái (P nhĩ)", "Lớn hai nhĩ"]
        )

        st.caption("Nhập biên độ để hệ thống tự động tính Sokolow-Lyon, Cornell & Romhilt-Estes:")
...
with col2:
    ...
    if uploaded:
        with st.spinner("Đang phân tích nhịp tim DII kéo dài và kiểm chuẩn toàn diện YDS 2026..."):
